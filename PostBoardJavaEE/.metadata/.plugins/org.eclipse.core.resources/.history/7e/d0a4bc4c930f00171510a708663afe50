...
public class PostService implements PostServiceLocal {
	...
	public List<PostModel> getAllPostsforDisplay() {
		String query = "select new PostModel(postmodel.id, postmodel.author, postmodel.title, postmodel.content, postmodel.date) from PostModel postmodel group by postmodel.id order by upper(postmodel.title) asc";
		Query q = entityManager.createQuery(query);
